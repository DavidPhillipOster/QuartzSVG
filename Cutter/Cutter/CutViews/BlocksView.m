...
- (void)drawParallelCuts:(CGRect)r {
  ...
  CGFloat w = floorf(right.size.width)/3;
  CGFloat h = floorf(right.size.height);
  BPath *p = [BPath bpath];
  CGFloat j = y;
  for (x += 2*w-kCutSpacingX;j+1 < y+2*h/3; j += kCutSpacingY, x += kCutSpacingX) {
    [p line2P:NSMakePoint(x, j) p:NSMakePoint(x+w, j)];
  }
  [currentContext openGroupNamed:@"a"];
  [p line2P:NSMakePoint(x-w-3.5*kCutSpacingX, j) p:NSMakePoint(x+w, j)];
  [currentContext closeGroup];

  x = ceilf(right.origin.x - r.size.width/3)+kCutSpacingX;
  j = y+h/3;
  float width = 2*w+4*kCutSpacingX;
  for (;j < y+h; j += kCutSpacingY, x += kCutSpacingX) {
    if (width != w) {
      [currentContext openGroupNamed:@"b"];
    }
    [p line2P:NSMakePoint(x, j) p:NSMakePoint(x+width, j)];
    if (width != w) {
      [currentContext closeGroup];
    }
    width = w;
